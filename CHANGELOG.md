...
- **剪贴板写入的簿记收成一处。** `write_clipboard_text` 和 `write_clipboard_image`
  各自维护一份「打开 → 清空 → 分配 → 拷贝 → 交出句柄 → 关闭」，两边都有容易出错的
  所有权规则（成功之后句柄归剪贴板，只能在自己失败的路径上释放）。现在都走
  `with_clipboard` + `set_clipboard_bytes`，关闭在**所有**路径上都会执行 ——
  剪贴板没关掉会挡住别的进程读写，表现像是整个系统的复制粘贴坏了。行为没有变化。

- **补上捕获路径的端到端测试。** 之前测的是碎片：DIB 解码、BMP 编码、写回剪贴板各测各的，
  但「从真实剪贴板读到图片 → 落盘 → 文件名能被界面查回来」这条完整链路没人测。
  新增 `clipboard::watcher::desktop` 四个用例，直接操作真实剪贴板：

  - 图片进去、BMP 和缩略图出来，且**和源字节逐位一致**
  - 文件名确实等于 `<内容哈希>.bmp` —— 不一致的话就是「捕获正常但永远显示不出来」
  - 剪贴板同时有文字和图片时，存的是文字
  - 空剪贴板不产生记录，也不落任何文件

- **CI 的测试过滤器从白名单改成 `--skip`。** 白名单得在每次加模块时手动更新，而且前缀
...
- **图片剪贴板。** 截图或复制图片现在会进历史。列表行显示缩略图，详情页显示原图，
  可以一键复制回剪贴板。之前复制图片是「什么都没发生」，连提示都没有。

  读的是 `CF_DIB`（V5 优先，回退到 CF_DIB），转成 BMP 落盘，WebView 直接渲染 ——
  全程没有图像编解码库，也就没多一个依赖，安装包体积没变。缩略图是 320px 的盒式
  降采样副本，跟原图一起写，所以列表行渲染时不需要解码或缩放。

  三个不显然的地方：

  - **32 位 `BI_RGB` 的第四个字节是丢掉的。** 截图通常是 32 位，而那个 alpha 字节
    一般是 0，渲染器会当成「全透明」—— 保留它得到的是一个看不见的图片。
  - **行宽按 4 字节对齐。** BMP 每行都补齐到 4 的倍数，解码和降采样都得按 stride 走，
    当成紧凑数组会把每行末尾的填充字节当成下一个像素。
  - **图片走「文本优先」。** 从浏览器或编辑器里复制，剪贴板经常同时有文字和位图，
    这时存文字 —— 那才是你想要的。

- **复制图片回剪贴板。** 详情页的「复制图片」，或者命令面板里对一条图片记录按回车。
  写出去的是 `CF_DIB` 而不是 `CF_BITMAP`：后者要建一个跟屏幕 DC 绑定的设备相关位图、
  再交给剪贴板接管，错一步很难查；认图片的消费者都认 DIB。写之前会把行序翻回
  自下而上、`biHeight` 改成正值 —— 负高度的 DIB 合法，但不少程序仍按老规矩读，
  粘贴出来是上下颠倒的，而这种 bug 只有肉眼看得出来。

- **图片条目不走 AI 动作。** 它没有文字。详情页那五个按钮换成一句说明加一个「复制图片」，
  而不是留一排点了没反应的灰按钮。

### Fixes

- **删掉的图片现在会被真正回收。** 图片文件按内容哈希命名，「清理已删除」时会一并删掉
  文件和它的缩略图。

  这里有个不显然的坑：`find_by_hash` 只看未删除的行，所以「复制一张图 → 删掉 → 再复制
  同一张」会产生两条记录、指向同一个文件。按行删文件会把还活着的那条也删掉。回收时的
  条件因此是「没有任何**存活**记录引用这个文件名」，而不只是「这条记录被删了」。
  这个 case 有专门的回归测试。

- **CI 测试过滤器修正。** 之前用 `clipboard::` 会把 `clipboard::writer` 的剪贴板往返用例
  也包进来 —— 那几个需要真实桌面会话，无 GUI 环境会挂住。现在显式列 `clipboard::watcher::`
  和 `clipboard::image::`，后者是纯字节操作，CI 上安全。

### Internal

- 缩略图缓存上限 40 条，按插入顺序淘汰，并且只在列表行滚进可视区时才请求 ——
  一张截图的 base64 有 8 MB 上下，200 行全量请求会让面板卡住。
- 新增 26 个测试（TypeScript 8 + Rust 18），含一个真实的剪贴板图片写入-读回往返，用来盯住行序。
  总计 282 个：TypeScript 186 + Rust 96（CI 上跑 91，差的是需要真实桌面会话的 5 个）。
